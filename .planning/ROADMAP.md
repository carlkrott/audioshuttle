...
### Phase 8: Hackathon Submission Prep 🔧 (IN PROGRESS)
**Goal:** Package AudioShuttle for Kaggle Gemma 4 Good Hackathon submission — security scrub, Docker containerization, GitHub repository, README, demo video, and full E2E pipeline verification with MCP + E4B multi-tool support.

**Plans:** 10 plans in 6 waves
- [x] 08-01-PLAN.md — Security scrub + .gitignore + codebase prep (Wave 1)
- [x] 08-02-PLAN.md — Docker container packaging (Wave 1)
- [ ] 08-03-PLAN.md — Test Dockerized setup (Wave 2, human-verify)
- [x] 08-04-PLAN.md — README + LICENSE + container docs (Wave 3)
- [x] 08-05-PLAN.md — GitHub repository + push (Wave 3)
- [ ] 08-06-PLAN.md — Demo video + Kaggle submission (Wave 5, human-action)
- [x] 08-07-PLAN.md — Fix multi-tool E4B translation + replay_command (Wave 1)
- [x] 08-08-PLAN.md — MCP transport verification (Wave 1)
- [x] 08-09-PLAN.md — E2E project build test (Wave 2)
- [ ] 08-10-PLAN.md — Demo video production: MCP preflight, Remotion intro/outro, BGM, screen recording (Wave 5)
...
| **E4B model doesn't emit multi-tool sequences** | "Create track called X" misses rename step | Fix system prompt with explicit examples + fix replay_command to use translate_multi() |
| **Bridge health probe incorrectly reports disconnected** | Misleading logs, no functional impact | Expected behavior — Reaper doesn't send OSC feedback for `/track/1/name` pings. Disconnection detection uses separate raw socket for actual health check. |
| **Docker audioshuttle + MCP stdio both create OSC bridges** | Two bridges sending to same Reaper port | UDP is stateless, no conflict. Feedback port (9000) conflict avoided because MCP stdio doesn't start feedback listener with --no-model flag? Verify in MCP transport plan. |
